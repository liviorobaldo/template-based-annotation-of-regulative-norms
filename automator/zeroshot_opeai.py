"""Zero-shot legal annotation helper.

This script sends the full legislative JSON payload to an OpenAI chat model using
an external prompt template stored in ``prompt.txt``. It is intentionally slim to
keep the workflow easy to maintain and customise.
"""
...
LOGGER = logging.getLogger("zeroshot_openai")
...
def build_user_prompt(template: str, *, document_json: str, source_path: Path) -> str:
...
def annotate_json(
    *,
    ...
    user_prompt = build_user_prompt(template, document_json=document_json, source_path=json_path)
    LOGGER.info("Submitting %s to model %s", json_path.name, model)
...
def run_cli(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="Zero-shot annotation over legislative JSON payloads")
...
if __name__ == "__main__":
    ...
    run_cli([
        "data/1989_41_part_1_sections_hierarchy.json",
        "--model", "gpt-4o",
